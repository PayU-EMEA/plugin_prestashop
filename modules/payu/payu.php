...
class Payu extends PaymentModule
{
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('Konto PayU');
        $this->description = $this->l('Akcepuje płatności przez serwis payu.pl .');
    ...
    public function postProcess()
    {
        ...
        if (Tools::isSubmit('submitPayUMainConf'))
        {
        ...
        if (Tools::isSubmit('submitPayUSandboxConf'))
        {
            $oauth_client_name_sandbox = (int)(Tools::getValue('oauth_client_name_sandbox'));
        ...
        if (Tools::isSubmit('submitPayUSecureConf'))
        {
            $oauth_client_name = (int)(Tools::getValue('oauth_client_name'));
        ...
        if (Tools::isSubmit('submitPayUGraphicsConf'))
        {
    ...
    public function getContent()
    {
        ...
        $output .= '<form action="'.$_SERVER['REQUEST_URI'].'" method="post">
					<fieldset><legend>'.$this->l('Main PayU configuration').'</legend>';
        $output .= '<label for="environment">'.$this->l('Environment').'&nbsp;&nbsp;</label>';
        $output .= '<select id="environment" name="environment">
						<option value="sandbox"'.(($this->payu_environment == 'sandbox') ? 'selected="selected"' : '').'>'.$this->l('Sandbox environment').'</option>
						<option value="secure"'.(($this->payu_environment == 'secure') ? 'selected="selected"' : '').'>'.$this->l('Secure environment').'</option>
					</select>
					<br /><br class="clear"/>';
        $output .= '<label for="self-return">'.$this->l('Is self-return enabled').'&nbsp;&nbsp;</label>';
        $output .= '<select id="self-return" name="self-return">
						<option value="1"'.(($this->payu_self_return == 1) ? 'selected="selected"' : '').'>'.$this->l('Enabled').'</option>
						<option value="0"'.(($this->payu_self_return == 0) ? 'selected="selected"' : '').'>'.$this->l('Disabled').'</option>
					</select>
					<br /><br class="clear"/>';
        $output .= '<label for="validity_time">'.$this->l('Validity time').'&nbsp;&nbsp;</label>';
        $output .= '<select id="validity_time" name="validity_time">
						'.$this->selectValidityTime($this->payu_validity_time).'
					</select>
					<br /><br class="clear"/>';
        $output .= '<label for="ship_abroad">'.$this->l('Ship to other country').'&nbsp;&nbsp;</label>';
        $output .= '<select id="ship_abroad" name="ship_abroad">
						<option value="true"'.(($this->payu_ship_abroad == true) ? 'selected="selected"' : '').'>'.$this->l('Enabled').'</option>
						<option value="false"'.(($this->payu_ship_abroad == false) ? 'selected="selected"' : '').'>'.$this->l('Disabled').'</option>
					</select>
					<br /><br class="clear"/>';
        $output .= '<label for="one_step">'.$this->l('One step checkout').'&nbsp;&nbsp;</label>';
        $output .= '<select id="one_step" name="one_step">
					...
					<br /><br class="clear"/>';
        $output .= '<input class="button" type="submit" name="submitPayUMainConf" value="'.$this->l('Save').'" style="margin-left: 400px;"/>
					</fieldset>
					</form><br class="clear"/>';

        $output .= '<form action="'.$_SERVER['REQUEST_URI'].'" method="post">
					<fieldset><legend>'.$this->l('Sandbox PayU configuration').'</legend>';
        $output .= '<label for="oauth_client_name_sandbox">'.$this->l('OAuth Client Name').'&nbsp;&nbsp;</label><input size="120" id="oauth_client_name_sandbox" type="text" name="oauth_client_name_sandbox" value="'.$this->payu_oauth_client_name_sandbox.'" />
					<br /><br class="clear"/>';
        $output .= '<label for="oauth_client_secret_sandbox">'.$this->l('OAuth Client Secret').'&nbsp;&nbsp;</label><input size="120" id="oauth_client_secret_sandbox" type="text" name="oauth_client_secret_sandbox" value="'.$this->payu_oauth_client_secret_sandbox.'" />
					<br /><br class="clear"/>';
        $output .= '<label for="pos_auth_key_sandbox">'.$this->l('Pos Auth Key').'&nbsp;&nbsp;</label><input size="120" id="pos_auth_key_sandbox" type="text" name="pos_auth_key_sandbox" value="'.$this->payu_pos_auth_key_sandbox.'" />
					<br /><br class="clear"/>';
        $output .= '<label for="signature_key_sandbox">'.$this->l('Signature Key').'&nbsp;&nbsp;</label><input size="120" id="signature_key_sandbox" type="text" name="signature_key_sandbox" value="'.$this->payu_signature_key_sandbox.'" />
					<br /><br class="clear"/>';
        $output .= '<input class="button" type="submit" name="submitPayUSandboxConf" value="'.$this->l('Save').'" style="margin-left: 400px;"/>
					</fieldset>
					</form><br class="clear"/>';

        $output .= '<form action="'.$_SERVER['REQUEST_URI'].'" method="post">
					<fieldset><legend>'.$this->l('Secure PayU configuration').'</legend>';
        $output .= '<label for="oauth_client_name">'.$this->l('OAuth Client Name').'&nbsp;&nbsp;</label><input size="120" id="oauth_client_name" type="text" name="oauth_client_name" value="'.$this->payu_oauth_client_name.'" />
					<br /><br class="clear"/>';
        $output .= '<label for="oauth_client_secret">'.$this->l('OAuth Client Secret').'&nbsp;&nbsp;</label><input size="120" id="oauth_client_secret" type="text" name="oauth_client_secret" value="'.$this->payu_oauth_client_secret.'" />
					<br /><br class="clear"/>';
        $output .= '<label for="pos_auth_key">'.$this->l('Pos Auth Key').'&nbsp;&nbsp;</label><input size="120" id="pos_auth_key" type="text" name="pos_auth_key" value="'.$this->payu_pos_auth_key.'" />
					<br /><br class="clear"/>';
        $output .= '<label for="signature_key">'.$this->l('Signature Key').'&nbsp;&nbsp;</label><input size="120" id="signature_key" type="text" name="signature_key" value="'.$this->payu_signature_key.'" />
					<br /><br class="clear"/>';
        $output .= '<input class="button" type="submit" name="submitPayUSecureConf" value="'.$this->l('Save').'" style="margin-left: 400px;"/>
					</fieldset>
					</form><br class="clear"/>';

        $output .= '<form action="'.$_SERVER['REQUEST_URI'].'" method="post">
					<fieldset><legend>'.$this->l('Graphics PayU configuration').'</legend>';
        ...
        $output .= '<select id="payment_accept" name="payment_accept">
					...
					<br /><br class="clear"/>';
        $output .= '<input class="button" type="submit" name="submitPayUGraphicsConf" value="'.$this->l('Save').'" style="margin-left: 400px;"/>
					</fieldset>
					</form><br class="clear"/>';

        $output .= '<form action="'.$_SERVER['REQUEST_URI'].'" method="post">
					<fieldset><legend>'.$this->l('Upgrade plugins').'</legend>
    ...
    public function execSuccessOrder($id_cart)
    {
        ...
        if($id_cart){
            ...
            $this->saveSID($payuSession->sid,(int)$this->currentOrder,'ORDER_STATUS_PENDING');
    ...
    {
        if(isset($_GET['error'])){
            Tools::redirectLink($this->myUrl . '/payment_error.php?error='.$_GET['error']);
        } else {
            if(isset($request)){
    ...
    public function execPayment($cart)
    {
        ...
        $isoLang = strtoupper(Language::getIsoById($cart->id_lang));

        $result = $this->orderCreateRequest($cart,$carriers,$isoLang);

        if(!empty($result)){
            $smarty->assign($result+array('id_customer'=>$cookie->id_customer));
            return $this->display(__FILE__, 'payment.tpl');
        } else {
            $smarty->assign(array('msg'=>$this->l('An error occurred while processing your order.')));
    ...
    public function execValidation()
    {
        ...
        if(!empty($result)){
            $smarty->assign(array('msg'=>$this->l('An error occurred while verifying the order.')));
            return $this->display(__FILE__, 'error.tpl');
        }
    }
    ...
    public function execError($code)
    {
        ...
        if(isset($code)){
            $smarty->assign(array('msg'=>$this->l('An error occurred during payment. Error Code:').' '.$code));
            return $this->display(__FILE__, 'error.tpl');
        }
    }
    ...
    public function hookPayment($params)
    {
        ...
        return $this->display(__FILE__, 'payment_payu.tpl');
    }
    ...
    {
        global $smarty, $protocol_content, $cookie;

        $img = Configuration::get('PAYU_IMG_ACCEPT');
        $img2 = Configuration::get('PAYU_IMG_ADVERT');
        if($cookie->id_lang != 6){
            $img = str_replace(array('/pl/','_pl.'),array('/en/','_en.'),$img);
            $img2 = str_replace(array('/pl/','_pl.'),array('/en/','_en.'),$img2);
        }
     ...
     */
    private function saveSID($sessionId,$orderId,$payUOrderStatus,$cartId = 0)
    {
        $sid = payu_session::existsByOrderIdAndSID($orderId,$sessionId);
        if($sid > 0){
    ...
    private function updateCustomerData($cartId)
    {
        ...
        if($cart->id_customer == 0){
            ...
            $customerRecord = $orderRetrieveResponse['CustomerRecord'];
            ...
            if(!empty($customerRecord) && !empty($shipping))
            {
                ...
                $address->address1 = $shipping['Address']['Street'].' '.$shipping['Address']['HouseNumber'].'/'.$shipping['Address']['ApartmentNumber'];
    ...
    private function orderCreateRequest($cart,$carriers,$isoLang)
    {
        ...
        $carrierList = array();
        $countryCode = Configuration::get('PS_LOCALE_COUNTRY');
    ...
    private function orderNotifyRequest($request)
    {
        ...
        if($result->getMessage() == 'OrderNotifyRequest')
        {
            ...
            $this->saveSID($sessionId,$orderId,$payUOrderStatus);
