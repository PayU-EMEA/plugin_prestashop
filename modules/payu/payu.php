...
class Payu extends PaymentModule
{
    ...
    public function execSuccessOrder($id_cart)
    {
        ...
        if ($id_cart) {
            ...
            $this->validateOrder($id_cart, Configuration::get('PAYMENT_PAYU_NEW_STATE'), $total, 'payu', 'payu.pl cart ID: ' . $id_cart . ', sessionId: ' . $payuSession->sid, null, null, false, $cart->secure_key);
            $this->saveSID($payuSession->sid, (int)$this->currentOrder, 'ORDER_STATUS_PENDING', $id_cart);
    ...
    private function orderCreateRequest($cart, $carriers, $isoLang)
    {
        ...
        $order = array(
            ...
            'OrderDescription' => $this->l('Order for cart: ' . $cart->id . ' from the store: ') . Configuration::get('PS_SHOP_NAME'),
